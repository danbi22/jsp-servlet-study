/**
 * function.js
 * 07_function.html의 기능을 담당하는 js파일 
 */
...
btnPlus.addEventListener('click', add(xValue, yValue));
btnMinus.addEventListener('click', minus(xValue, yValue));
btnMulti.addEventListener('click', multi(xValue, yValue));
btnDivide.addEventListener('click', divide(xValue, yValue));


function add(xValue, yValue){
    resultMain.innerHTML = x + y;
};
...
result = add(1, 2, 3); // 함수 선언보다 더 많은 argument를 전달.
console.log('result =' + result);

result = add(1); // 함수 선언의 parameter 개수보다 적은 argument를 전달
// 전달하지 않은 파라미터는 undefined가 됨.
...
function divide(xValue, yValue){
    resultMain.innerHTML = x / y;
};

function multi(xValue, yValue){
    resultMain.innerHTML = x * y;
};
...
// const plus = add(); -> 함수의 리턴 값을 plus에 저장
result = plus(100, 200);
console.log('result: ', result);


// 익명 함수: 이름이 없는 함수
const minuss = function (x, y) {
   return x - y; 
};

console.log('minus', minuss(1, 2));

// 익명 함수를 선언과 동시에 호출:
result = (function (x, y) { // 이름 없는 함수
    return x/y;
})(1, 2); //(1, 2) 아규먼트 1, 2를 주고 호출
console.log('result =', result);
