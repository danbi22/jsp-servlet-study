...
public class ForwardSevlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    System.out.println("ForwardSevlet#doGet() 호출");
	    
	    // 클라이언트에서 서버로 온 요청을 새로운 페이지로 "forward" 방식으로 이동:
	    // 같은 WAS(Web Application Server)의 같은 웹 어플리케이션 안에서만 페이지 이동이 가능.
	    // 요청 주소(URL, http://localhost:8081/jsp1/ex3)가 바뀌지 않음. 
	    // request 객체와 response 객체가 유지됨
	    // 다른 (외부) 서버 또는 다른 웹 어플리케이션의 페이지로 forward 이동을 할 수 없음.
